...
app.get('/inscription', function (req, res) {
    var sport, objectif, co = connection();
    co.connect();
    co.query("select * from sport;", function (error, results, fields) {
        ...
        co.query("select * from objectifs;", function (error, results, fields) {
            if (error) return console.log(error);
            objectif = results;
            res.render('register.twig', {sports: sport, objectifs: objectif});
        })
    })
});
...
// récupérer les infos du user en fonction de l'id contenu dans req.session
    ...
    var user_id = req.session.someAttribute, co = connection();
    co.connect();
...
app.post('/inscription', function (req, res) {
    let q = "select * from users where email like '" + req.body.email + "';", co = connection();
    co.connect();
